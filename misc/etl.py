...
class ETL:
    """
    ETL class for all data operations pre model inputs.
    Data is read in generative way to allow for large datafiles and low memory utilization    
    """
    
    def generate_clean_data(self, filename, batch_size=1000, start_index=0):
        while h5py.File(filename, 'r') as hf:
            i=start_index
            while True:
    ...
    def create_clean_datafile(self, filename_in, filename_out, batch_size=1000, 
                              x_window_size=100, y_window_size=1, y_col=0, filter_cols=None, normalise=True):
